var Init = {
  init: function(){
    ...
    $('#content').css('border-top-right-radius',border_radius+'px');
    $('#content').css('border-top-left-radius', border_radius+'px');
    
    var h_w_margin_ratio = 1;
    var width = $('#content').position().left*h_w_margin_ratio;
    var border_width = parseInt($('#content').css('border-left-width'));
    $('#top').height(width-30);
    ...
    $('#content').height(total_height-width-30);
    
    var midground  = '#DC3D24';//red
    var background = '#232B2B';//grey
    var foreground = '#232B2B';
    $('#top').css('color',midground);
    $('#content').css('border-color',midground);
    $('#content').css('background-color',foreground);
